...
from database import (
    ...
    login_user, register_user, reset_password as db_reset_password,
    validate_token, get_user_by_token,
)
...
async def reset_password_endpoint(data: PasswordResetInput):
    if not data.username.strip() or not data.new_password.strip():
        return JSONResponse({"error": "Username and new password required"}, status_code=400)
    if len(data.new_password) < 6:
        return JSONResponse({"error": "New password must be at least 6 characters"}, status_code=400)

    if not db_reset_password(data.username.strip(), data.new_password):
